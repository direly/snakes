...
direct_sw = True
def SnackAiDirect():
    global snack, next_direct, direct_sw
    i, j = snack[0][0], snack[0][1]
    if max_l < 2 or max_c < 2:
        return
    ...
    if max_l % 2 == 1 and max_c % 2 == 1:
        if j < max_c - 2:
            ...
            if i == 0 and j == max_c - 2:
                next_direct = 3
            if i == 0 and j == max_c - 1:
                next_direct = 3
            if i == 1 and j == max_c - 2:
                next_direct = 1
            if i == 1 and j == max_c - 1:
                if direct_sw:
                    next_direct = 3
                else:
                    next_direct = 1
            direct_sw = not direct_sw
    return 
