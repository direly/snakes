...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_UP and direct != 2:
                next_direct = 1
            elif event.key == pygame.K_DOWN and direct != 1:
                next_direct = 2
            elif event.key == pygame.K_LEFT and direct != 4:
                next_direct = 3
            elif event.key == pygame.K_RIGHT and direct != 3:
                next_direct = 4

    cur_time = GetTick()
    if timer != cur_time:
        timer = cur_time
        if status == 4 and next_direct > 0:
            status = SnackMove()
            if 1 == status:
                print "win"
            elif 2 == status:
